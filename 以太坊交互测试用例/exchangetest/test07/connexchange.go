...
func BuildDepositOrder()  {
	...
	auth.GasLimit=3000000
	auth.Value = big.NewInt(10000000)
	tx, err := tokenme.DepositsOrder(auth, 10000001, common.HexToAddress(address_test_01), address_test_01, true, big.NewInt(100000000),big.NewInt(100), common.HexToAddress(address_nill), big.NewInt(time.Now().Unix()))
	if err != nil {
		fmt.Println("exchang合测试失败：", err)
		return
	}
