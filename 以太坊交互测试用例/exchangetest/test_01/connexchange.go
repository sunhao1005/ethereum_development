...
func ExchangeTestOrderError() {
	...
	tx, err := tokenme.DepositsSell(auth, 10001, common.HexToAddress(address_exchange_owner), common.HexToAddress(address_exchange_owner), true, big.NewInt(10000000), big.NewInt(100), common.HexToAddress(address_nill), big.NewInt(time.Now().Unix()))
	if err != nil {
		fmt.Println("exchang合测试失败：", err)
		return
	}
