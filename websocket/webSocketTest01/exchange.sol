...
contract exchange{
  ...
  //
   ...
   event DepositsSell(uint64 orderid, string receiver,bool tradetype, uint256 sellamount,address selladdress,uint256 price,uint256 buyamount,uint256 createTime);
   ...
   event GetMoney(address owner,uint256 money,uint256 time);
    ...
    function depositsOrder(uint64 orderid,
       ...
       require(orderidToUsers[orderid].create_at==0&&orderidToUsers[orderid].user_address==address(0)&&orderidToUsers[orderid].sell_amount==0);
       require(useraddress==msg.sender);
       require(bytes(receiver).length!=0);
       require(startContract==true);
       require(sellamount==msg.value&&sellamount>0);
       if(tradetype==false){
    ...
    function depositsSell(uint64 orderid,
                      ...
                      uint256 price,
                      uint256 createTime
                      )checkowner(msg.sender) public returns(bool) {
         require(orderidToUsers[orderid].create_at==0&&orderidToUsers[orderid].user_address==address(0)&&orderidToUsers[orderid].sell_amount==0);
    ...
    //withdrawal
    function withdrawal(uint64 txid) public returns(bool){
       require(startContract==true);
    ...
    function changeContractAddress(address addr,uint64 tokenid,string types,uint64 decimals,bool chnageType)checkowner(msg.sender)public{
           require(bytes(types).length>0);
           require(tokenid>0);
           require(addr!=address(0));
           contractTypeAddress[addr]=chnageType;
           tokens memory token=tokens(tokenid,types,addr,decimals,chnageType);
           tokenTable[tokenid]=token;

    }

   function getTokenInfor(uint64 tokenid)checkowner(msg.sender) view public returns(uint64,string,address,uint64,bool) {
       ...
       uint64 d =tokenTable[tokenid].decimals;
       bool   e =tokenTable[tokenid].iscancl;
       return (a,b,c,d,e);
  }
